...
class StrangersExpressionThread(threading.Thread):
    ...
    def get_frame(self):
        ...
        if self.global_frame is not None:
            ret, jpeg = cv2.imencode('.jpg', self.global_frame)
            return jpeg.tobytes()
    ...
    def run(self):
        ...
        strangers_resend_time = 60  # if >= 60 seconds, then resend message.
        stranger_exist = False
                ...
                if 'Unknown' in names:  # alert
                    if strangers_timing == 0:  # just start timing
                        ...
                        if difference < strangers_limit_time:
                            ...
                            if not stranger_exist:
                                thread = Thread(target=http_post, args=(self.url, 2, event_location, event_desc))
                                thread.start()
                                stranger_exist = True
                            if stranger_exist and difference > strangers_resend_time:
                                thread = Thread(target=http_post, args=(self.url, 2, event_location, event_desc))
                                thread.start()
                                strangers_start_time = time.time()
                            ...
                            # adjust to servo
                            if direction:
                                print('%d-摄像头需要 turn %s %d 度' % (counter, direction, self.ANGLE))
